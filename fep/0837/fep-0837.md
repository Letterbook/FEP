...
Proposals can be linked to actors (if actor provides a service) or to other objects (if they represent economic resources) using [FEP-0ea0](https://codeberg.org/fediverse/fep/src/branch/main/fep/0ea0/fep-0ea0.md) payment links. Proposals can also be added to public collections, or be delivered to actor's followers using `Create` activity, or announced by group actors.
